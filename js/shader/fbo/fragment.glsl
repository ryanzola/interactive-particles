...
void main() {
  ...
  // sine wave projected onto a circle
  float frequency = 1.0;
  float amplitude = 2.0;
  float targetRadius = mix(
    info.x, 
    amplitude, 
    0.5 + 0.45 * sin(angle * frequency + time * 0.2)
  );
  ...
  // the difference between the original position and the target position
  // not deliberately setting the position of the particles but rather
  // creating the force that will move the particles in a circle
  pos.xy += (targetPos.xy - pos.xy) * 0.1;

  pos.xy += curl(pos.xyz*2.0, time * 0.1, 0.1).xy * 0.006;

  // calculate the distance between the mouse and the particle
  float distance = length(pos.xy - mouse * vec2(0.85));

  // calculate the direction from the mouse to the particle
  vec2 dir = normalize(pos.xy - mouse);
	
  pos.xy += dir * 0.1 * (1.0 - smoothstep(0.0, 0.5, distance));
